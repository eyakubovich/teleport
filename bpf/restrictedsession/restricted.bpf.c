...
BPF_LPM_TRIE(ip4_denylist, struct ip4_trie_key, u8, 128);

// IPv4 addresses (with masks) to allow (override deny)
BPF_LPM_TRIE(ip4_allowlist, struct ip4_trie_key, u8, 128);

// IPv6 addresses (with masks) to deny
BPF_LPM_TRIE(ip6_denylist, struct ip6_trie_key, u8, 1024);

// IPv6 addresses (with masks) to allow (override deny)
BPF_LPM_TRIE(ip6_allowlist, struct ip6_trie_key, u8, 1024);
...
BPF_RING_BUF(audit_events, 4*4096);
